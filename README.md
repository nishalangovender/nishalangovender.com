...
- **3D hero** — `src/components/sections/hero-scene/` fills the homepage hero behind the headline and tells the notebook-to-factory story as one three.js scene. A kinematic diagram (world and body axes on the dot grid, heading θ, velocity V, ω) is written in a neat hand — slightly wobbly pen strokes, handwritten symbols — on the dotted right-hand page of an open, stitched hardcover notebook on a walnut desk, beside a heavy aluminium mechanical keyboard and an ergonomic mouse. The robot's outline lifts into a wireframe AGV while the annotations stay on the page, and its electronics appear as wireframes in an exploded view above the chassis — battery, motor drivers, Pi — then collapse into place and are cabled. The camera pulls back to the monitor behind the notebook, where `nish_bot` builds and launches in a terminal and the AGV's status LEDs light as it boots. The AGV drives off the page, its wheels rolling — castor dipping off the notebook first — and parks at the desk's edge; the whole desk then shrinks away beneath it, leaving the robot, now solid and full size, on a concrete factory floor, while a lidar point cloud resolves into the production floor around it — racking with loaded cartons, lane lines, walls and columns, lit with image-based light and filmic tone mapping — with the scan and an amber costmap kept as an RViz overlay. The camera follows the AGV in, pulls up over the fleet and flies into the monitor, where the production dashboard runs live, then pulls back to the notebook, where the written sheet lifts at its corner and curls over the spine onto the left-hand stack, uncovering a fresh page. The loop runs at an unhurried 39 s. Once the factory has formed, click or tap the floor during the deploy and system beats to send a nav goal: the loop pauses and pure pursuit drives there along the drawn path. The scene loads after the hero text, pauses offscreen, and falls back to a static notebook frame for reduced motion or no WebGL. In development, `?beat=1`–`6` holds the loop on one beat and `?t=S` freezes it at S seconds.
