...
- **3D hero** — `src/components/sections/hero-scene/` tells the notebook-to-factory story as one three.js scene: a kinematic diagram (world and body axes, heading θ, velocity V, ω) is inked on a dot-grid page, lifts into a wireframe AGV, and `nish_bot` builds and launches in a terminal. The page's dots then rise into a lidar point cloud of a factory where the AGV runs its mission with a laser scan and an amber costmap. The camera pulls up over the fleet and flies into a monitor showing the production dashboard, drawn live from the fleet. Once the factory has formed, click or tap the floor during the deploy and system beats to send a nav goal: the loop pauses, pure pursuit drives there, and `/cmd_vel` prints below. The scene loads after the hero text, pauses offscreen, and falls back to a static notebook frame for reduced motion or no WebGL. In development, `?beat=1`–`6` holds the loop on one beat.
