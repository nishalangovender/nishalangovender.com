 ...
 * metres; heights above the factory floor). The notebook lies on the desk and
 * the monitor stands behind it. The AGV drives off the page and parks at the
 * desk's edge; the whole desk then shrinks away beneath it to a point on the
 * floor (see `deskScale` in mission.ts), leaving the robot, full size, on the
 * factory floor.
 */
...
export const DESK = { minX: -7.2, maxX: 3.6, minY: -2.4, maxY: 3.9, height: 2.2, thickness: 0.16 } as const;
...
/**
 * Where the AGV parks at the desk's edge (drive axle, castor still on the
 * desk), and the floor point beneath it that the desk shrinks towards.
 */
export const PIVOT = { x: DESK.maxX - 1, y: 0 } as const;
 ...
 * board's rim, the desk, or the factory floor. `deck` is the desk's scale
 * (1 full size, 0 shrunk away): the AGV only rides it down while parked at
 * the pivot, so heights scale with it and the footprint is left as is.
 */
export function surfaceHeight(x: number, y: number, deck = 1): number {
  if (deck <= 0 || x > DESK.maxX || y < DESK.minY || y > DESK.maxY) return 0;
  if (x <= COVER_EDGE && Math.abs(y) <= PAGE.depth / 2 + NOTEBOOK.coverMargin) {
    const onPages = x <= PAGE_EDGE && Math.abs(y) <= PAGE.depth / 2;
    return deck * (onPages ? PAGE_HEIGHT : DESK.height + NOTEBOOK.cover);
  }
  return deck * DESK.height;
}

/** Where the surface steps along the lane — a rolling wheel can rest on these corners. */
const CORNERS = [PAGE_EDGE, COVER_EDGE, DESK.maxX];
 ...
 */
export function wheelCentreHeight(x: number, y: number, r: number, deck = 1): number {
  const at = (sx: number) => surfaceHeight(sx, y, deck) + Math.sqrt(Math.max(0, r * r - (x - sx) ** 2));
  let best = at(x);
 ...
 * first (the nose dips), then the drive wheels follow. `deck` is the desk's
 * scale (see surfaceHeight). `height` is the ground under the drive axle;
 * `pitch` is nose-down, radians.
 */
export function bodyPose({ x, y, theta }: Pose, deck = 1): { height: number; pitch: number } {
  const rear = wheelCentreHeight(x, y, AGV.wheelRadius, deck);
  ...
  const gap = (phi: number) => {
    ...
    return castor - wheelCentreHeight(x + ahead * Math.cos(theta), y + ahead * Math.sin(theta), AGV.castorRadius, deck);
  };
